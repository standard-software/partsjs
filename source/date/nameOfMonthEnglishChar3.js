...
export const nameOfMonthEnglishChar3 = (date, isLocal = true) => {
  ...
  if (!isDate(date)) {
    throw new TypeError(
      `nameOfMonthEnglishChar3 args(date:${date}) is not date`,
    );
  }
  if (!isBoolean(isLocal)) {
    throw new TypeError(
      `nameOfMonthEnglishChar3 args(isLocal:${isLocal}) is not boolean`,
    );
  }
...
export default { nameOfMonthEnglishChar3 };
